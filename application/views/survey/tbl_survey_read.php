<!doctype html>
<html>
    <head>
        <title>harviacode.com - codeigniter crud generator</title>
        <link rel="stylesheet" href="<?php echo base_url('assets/bootstrap/css/bootstrap.min.css') ?>"/>
        <style>
            body{
                padding: 15px;
            }
        </style>
    </head>
    <body>
        <h2 style="margin-top:0px">Tbl_survey Read</h2>
        <table class="table">
	    <tr><td>Nama Kk</td><td><?php echo $nama_kk; ?></td></tr>
	    <tr><td>No Kk</td><td><?php echo $no_kk; ?></td></tr>
	    <tr><td>Tanggal Survey</td><td><?php echo $tanggal_survey; ?></td></tr>
	    <tr><td>Nama Petugas</td><td><?php echo $nama_petugas; ?></td></tr>
	    <tr><td>Jumlah Tanggungan</td><td><?php echo $jumlah_tanggungan; ?></td></tr>
	    <tr><td>Penghasilan</td><td><?php echo $penghasilan; ?></td></tr>
	    <tr><td>Luas Rumah</td><td><?php echo $luas_rumah; ?></td></tr>
	    <tr><td>Listrik</td><td><?php echo $listrik; ?></td></tr>
	    <tr><td>Sumber Air</td><td><?php echo $sumber_air; ?></td></tr>
	    <tr><td></td><td><a href="<?php echo site_url('survey') ?>" class="btn btn-default">Cancel</a></td></tr>
	</table>
        </body>
</html>